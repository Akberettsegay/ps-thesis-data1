...
$(foreach ($conn in ($ReplicationConnections | Sort-Object ToSite, ToServer)) {
    ...
    if ($conn.OverrideNotify -and $conn.NotifyEnabled) {
        ...
        # Inter-site KCC connection without notify override: site link lacks USE_NOTIFY
    ...
    $siteDisplay = if ($conn.IsInterSite) { "$($conn.FromSite) &rarr; $($conn.ToSite)" } else { $conn.ToSite }
@"
                <tr>
