...
Function Start-ETLTrace
{
	...
	if ($GetAdvisor)
	{
		...
		$TracingFiles | ForEach-Object { Move-Item "$installdir`\$_" $TempDirectory }
	}
	...
	if ($GetAPM)
	{
		...
		$APMTracingFiles | ForEach-Object { Move-Item "$installdir`\$_" $TempDirectory }
	}
...
")
	...
	Function Start-ScomETLTrace
	{
		...
		if ($NetworkTrace)
		{
			...
			netsh trace start capture=yes persistent=yes overwrite=yes scenario=netconnection tracefile="$TempETLTrace`\Network Trace\$mod.etl" fileMode=circular maxSize=2048 | Out-Null
