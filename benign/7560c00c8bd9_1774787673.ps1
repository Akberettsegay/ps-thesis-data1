...
AMET
    ...
    #  You can generate a compatable CSV file by adding these columns in the https://backup.management console and then using the Export option to get and XLSX file
    #  Then same the XLS file as a CSV

    <# Sample CSV Format

    Device name,Installation key,
    2022-man_hpw2c,7f86da88730e
    2022-lis_drbpu,3828260aae8b
    2022-dus_hwf7d,7b7a6b322b3b
    2022-doh_gvsnj,6b0fa0e7f6e3
    2022-ayt_ki9yx,e3ad88720eaf
    2022-hel_ol09i,88b88ab668fa
    2022-klp_tzh5r,66a2c22a6c8b
    2022-mco_224ln,6b870c68822d
    2022-jfk_2g73d,f0fae23d827b
    
    #>
# -----------------------------------------------------------#>  ## Behavior

[CmdletBinding(DefaultParameterSetName="Local")]                                                                     ## Change between Network, Local or Help to assign a default 
    Param (
        ...
        [Parameter(ParameterSetName="Local",Mandatory=$False)] [String]$LocalPath = "D:\OffsiteVault-new",                        ## Set a local OffsiteSpeedVault target
        ...
        [Parameter(Mandatory=$False)][switch]$SelectDevices,                                                            ## $True to prompt for Device Selection
        [Parameter(Mandatory=$False)][string]$DeviceList = "D:\devicelist1.csv"                                         ## Path to CSV file with devicename and devicepassword
...
#region ----- Functions ----
    ...
    Function Start-SeedDownload {
        ...
        foreach ($Device in $Selected) {
            ...
            for($i = 0; $i -le 100; $i = ($i + 1) % 100) {      # Loop device list
    ...
    Switch ($PSCmdlet.ParameterSetName) { 
        'Network' {
            ...
            if(!(Test-Path -Path $OffsiteVaultRootPath)) {
                Write-output "  OffsiteSpeedVault Root Path ( $OffsiteVaultRootPath ) does not exist or is not accessible"
            ...
            if (Test-Path -Path $DeviceList) {
                ...
                Write-Warning "  $devicelist not found"
                break
            }
