...
<# ----- Behavior: ----
    ...
    #
    #  Suggested use with Powershell Script Monitors (Connectwise or other)
    #  Triggering on Script Output 'Contains' Keyword 'Warning:'
    #  Warnings force an 'Exit 1001' Code for user with N-able N-sight RMM
    #  'Exit ###' Code could be adjusted for other RMM use cases.
    #
 # -----------------------------------------------------------#>
    ...
    Switch ($script:datasource) { ## "FileSystem","SystemState","VMware","VssHyperV","VssMsSql","Exchange","MySql","NetworkShares","VssSharePoint"
        ...
        'VssMsSql' {
            ...
            if ($DatasourcePresent ) {
                $DatasourcePresent | Format-Table
                Get-BackupSelections
                Get-StatusReport
            }
            else{
                Write-Warning "No $Datasource installation detected"; <#$global:failed = 1#>}
            }
        'Exchange' {
