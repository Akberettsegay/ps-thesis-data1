...
<# ----- Behavior: ----
    ...
    # Use the -ExportHistory switch parameter to export backup and restore history
    # Use the -HistoryMonths ## parameter to define the number of months to return
...
[CmdletBinding(DefaultParameterSetName="Export")]
    Param (
        [Parameter(ParameterSetName="Export",Mandatory=$False)] [switch]$AllPartners = $true,              ## $AllPartners = $true, to Skip GUI partner selection
        [Parameter(ParameterSetName="Export",Mandatory=$False)] [switch]$AllDevices = $true,               ## $AllDevices = $true, to Skip GUI device selection
        ...
        [Parameter(ParameterSetName="Export",Mandatory=$False)] [switch]$ExportHistory = $false,             ## Generate individual Domain XLS/CSV output files with session history
        [Parameter(ParameterSetName="Export",Mandatory=$False)] [int]$HistoryMonths = 3,                   ## Set number of months of device session history to return
...
#region ----- Environment, Variables, Names and Paths ----
    ...
    Write-output "  -ExportHistory   = $ExportHistory"
...
#region ----- Backup.Management JSON Calls ----
    ...
    Function EnumerateM365History ($AccountToken) {
        ...
        $data.jsonrpc = '2.0'
        $data.id = '2'
        $data.method = 'EnumerateSessions'
        $data.params = @{}
        $data.params.accountToken = $accountToken
        $data.params.filter = @{}
        $data.params.filter.CreatedAfter = Convert-DateTimeToUnixTime ((Get-Date).addmonths([int]$historymonths * -1))
        $data.params.filter.CreatedBefore = Convert-DateTimeToUnixTime ((Get-Date).adddays(1))
        $data.params.range = @{}
        $data.params.range.Offset = 0
        $data.params.range.Size = 20000
        ...
        $Script:EnumerateM365HistoryResponse = Invoke-RestMethod @params 

        $script:M365Sessions = $EnumerateM365HistoryResponse.result.result | Select-object @{N="Partner";E={$device.partnername}},
                                                                                                    @{N="Account";E={$device.DeviceName}},
                                                                                                    Id,
                                                                                                    Type,
                                                                                                    @{N="DataSource";E={$_.DataSourceType}},
                                                                                                    @{N="StartTime";E={ Convert-UnixTimeToDateTime ($_.StartTime)}},
                                                                                                    @{N="EndTime";E={ Convert-UnixTimeToDateTime ($_.EndTime)}},
                                                                                                    @{N="Duration";E={[MATH]::Round(($_.Duration/60),2)}},
                                                                                                    @{N="Users";E={$_.AccountsCount}},
                                                                                                    @{N="Sites";E={$_.SiteCollectionsCount}},
                                                                                                    Status,
                                                                                                    ErrorsCount,
                                                                                                    @{N="AccountToken";E={$accounttoken}} 

        $Script:M365Sessions | Select-object * | format-table
        if ($gridview) {$Script:M365Sessions | out-gridview -title "$($device.partnername) | $($device.DeviceName)" }
    ...
    Switch ($PSCmdlet.ParameterSetName) { 
        'Import' {
            ...
            if ($importpath) {
                ...
                $M365UserSelectionCSVFile = import-Csv -Path $OpenFileName
            }
        ...
        'Export' { 
            ...
            foreach ($device in $SelectedDevices) {
                ...
                If ($ExportHistory) {
                    EnumerateM365History $device.AccountToken
                    $Script:csvoutputfile4 = "$ExportPath\$($CurrentDate)_M365_History_Export_$($device.Partnername -replace(`" \(.*\)`",`"`") -replace(`"[^a-zA-Z_0-9]`",`"`"))_$($device.DeviceName)_$($device.PartnerId).csv"
                    $Script:M365Sessions | Select-object * | Export-CSV -Delimiter $delimiter -path "$csvoutputfile4" -NoTypeInformation -Encoding UTF8
                }
            ...
            If ($ExportIndividual -or $ExportHistory) {
                Write-output $Script:strLineSeparator
