...
$LabName = 'SCOM2012R2' # (Default: SCOM2012R2)
...
########### Additional Settings ##############
##############################################
$SCOMSetupLocalFolder = 'C:\System Center Operations Manager 2012R2' #Where you would like to extract the SCOM Installation when installing the SCOM Components to the Management Server. (Default: C:\System Center Operations Manager 2012R2)
$SQLSetupLocalFolder = 'C:\SQL Server 2012 SP1' #Where you would like to extract the SQL Installation. (Default: C:\SQL Server 2012 SP1)
$WindowsOperatingSystem = 'Windows Server 2012 R2 Datacenter Evaluation (Server with a GUI)' #Set the Operating System Type here. Grab the OS with this command: Get-LabAvailableOperatingSystem
$NumberOfManagementServers = 3
[double]$HyperV_Memory = 4GB #Default memory for All Servers (Default: 4GB)
[double]$HyperV_MinMemory = 2GB #Default min memory for All Servers (Default: 2GB)
[double]$HyperV_MaxMemory = 4GB #Default max memory for All Servers (Default: 4GB)
[double]$HyperV_HighestMemory = 8GB #Change this to whatever you want the max memory to be for your lab (This is the Memory Set for the Management Server and SQL) (Default: 8GB)
...
# Hostname | Role
# DC-2012R2 : Domain Controller
# MS01-2012R2 : System Center Operations Manager 2012R2 - Management Server
# MSSQL-2012R2 : SQL Server 2012R2
# IIS-2012R2 : IIS Windows Agent
# RHEL7-9 : Redhat 7.9

##############################################
########### Network / IP Address #############
##############################################
$NetworkID = '192.170.0.0/24' # (Default: 192.170.0.0/24)
$DC2012R2IPv4Address = '192.170.0.1' # (Default: 192.170.0.1)
$GatewayIPv4Address = '192.170.0.2' # Hyper-V Host IP (Default: 192.170.0.2)
$SQL2012R2IPv4Address = '192.170.0.11' #SQL 2012R2 Server (Default: 192.170.0.11)
$IIS_IPv4Address = '192.170.0.21' #SCOM Web Console (Default: 192.170.0.21)
$SCOM2012R2MSIPv4Address = '192.170.0.31' # (Default: 192.170.0.31)
#Redhat 7.9 - if you dont want this deployed, set $DeployRHEL79 = $false.
$DeployRHEL79 = $false # (Default: $false)
$RHEL79IPv4Address = '192.170.0.58' # (Default: 192.170.0.58)
...
############### Product Keys #################
##############################################
#Currently written to only license DataCenter Evaluation version: https://www.microsoft.com/en-us/evalcenter/evaluate-windows-server-2012R2
...
$CustomDomainAdmin = 'bdrumm' #Additional Domain Admin to add to installation. (Default: $null)
...
$SCOMMgmtGroup = 'SCOM-2012R2-MG' # SCOM management group name (SCOM-2012R2-MG, for example) (Default: SCOM-2012R2-MG)
#SQL Accounts
...
#For Microsoft.Windows.Server.2012R2.Discovery and Microsoft.Windows.Server.Library
...
$SCOMIISManagementPackURI = 'https://download.microsoft.com/download/9/1/1/911CF75A-3950-4853-9F9E-E54CFC86075E/Microsoft%20System%20Center%20Management%20Pack%20for%20IIS%202012.msi'
$ReportViewer2012RuntimeURI = 'https://download.microsoft.com/download/F/B/7/FB728406-A1EE-4AB5-9C56-74EB8BDDF2FF/ReportViewer.msi'
$SystemCLRTypesForSQLServer2012x64URI = 'https://download.microsoft.com/download/F/E/D/FEDB200F-DE2A-46D8-B661-D019DFE9D470/ENU/x64/SQLSysClrTypes.msi'
...
#SQL Server 2012R2 : This will automatically grab the latest SQL Server 2012R2 Cumulative Update Download URL from the Microsoft Website.
#$SQLServer2012LatestCUURI = ((Invoke-WebRequest -Uri "https://www.microsoft.com/en-us/download/confirmation.aspx?id=56975" -UseBasicParsing).Links | Where { $_ -like "*SQLServer2012-KB*-x64.exe*" })[0].href
...
#$GatewayIPv4Address = (Get-NetIPInterface | Where{ $_.InterfaceAlias -like '*SCOM2012R2*' } | Where{ $_.AddressFamily -eq 'IPv4' } | Get-NetIPAddress).IPAddress

$IISAgentNetAdapter = @()
$IISAgentNetAdapter += New-LabNetworkAdapterDefinition -VirtualSwitch $LabName -Ipv4Address $IIS_IPv4Address -Ipv4Gateway $GatewayIPv4Address -Ipv4DNSServers $DC2012R2IPv4Address -RegisterInDNS:$true
if ($UseDefaultSwitch)
{
	$IISAgentNetAdapter += New-LabNetworkAdapterDefinition -VirtualSwitch 'Default Switch' -UseDhcp
}
$SQL2012R2NetAdapter = @()
$SQL2012R2NetAdapter += New-LabNetworkAdapterDefinition -VirtualSwitch $LabName -Ipv4Address $SQL2012R2IPv4Address -Ipv4Gateway $GatewayIPv4Address -Ipv4DNSServers $DC2012R2IPv4Address -RegisterInDNS:$true
if ($UseDefaultSwitch)
{
	#Adding an Internet Connection on the SQL Server (Required for the SQL Setup via AutomatedLab)
	$SQL2012R2NetAdapter += New-LabNetworkAdapterDefinition -VirtualSwitch 'Default Switch' -UseDhcp
}

$SCOM2012R2MSNetAdapter = @{ }
[int]$i = 0
...
do
{
	...
	$i++
	[string]$i = "{0:D2}" -f $i
	$SCOM2012R2MSNetAdapter["var$i"] = @()
	$MSIPv4Address = ((($SCOM2012R2MSIPv4Address).Split(".") | Select -First 3) -join '.') + "." + ([int](($SCOM2012R2MSIPv4Address).Split(".") | Select -Last 1) + $i)
	$SCOM2012R2MSNetAdapter["var$i"] += New-LabNetworkAdapterDefinition -VirtualSwitch $LabName -Ipv4Address $MSIPv4Address -Ipv4Gateway $GatewayIPv4Address -Ipv4DNSServers $DC2012R2IPv4Address -RegisterInDNS:$true
	if ($UseDefaultSwitch)
	{
		#Adding an Internet Connection on the Management Server
		$SCOM2012R2MSNetAdapter["var$i"] += New-LabNetworkAdapterDefinition -VirtualSwitch 'Default Switch' -UseDhcp
	}
	[int]$i = $i
}
until ($i -eq $NumberOfManagementServers)
if ($DeployRHEL79)
{
	$RHEL79NetAdapter = @()
	$RHEL79NetAdapter += New-LabNetworkAdapterDefinition -VirtualSwitch $LabName -Ipv4Address $RHEL79IPv4Address -Ipv4Gateway $GatewayIPv4Address -Ipv4DNSServers $DC2012R2IPv4Address -RegisterInDNS:$true
	if ($UseDefaultSwitch)
	{
...
if ($SQLProductKey)
{
	$SQLServer2012Role = Get-LabMachineRoleDefinition -Role SQLServer2012 -Properties @{
...
Add-LabIsoImageDefinition -Name SQLServer2012 -Path $labSources\$SQLServerISO

#region server definitions
#Root Domain controller
Add-LabMachineDefinition -Name DC-2012R2 -Roles RootDC -IpAddress $DC2012R2IPv4Address -Gateway $GatewayIPv4Address -DnsServer1 $DC2012R2IPv4Address
#SCOM server
[int]$i = 0
do
{
	...
	$i++
	[string]$i = "{0:D2}" -f $i
	Add-LabMachineDefinition -Name "MS$i`-2012R2" -NetworkAdapter $SCOM2012R2MSNetAdapter["var$i"] -Memory "$HyperV_HighestMemory" -MinMemory "$HyperV_MinMemory" -MaxMemory "$HyperV_HighestMemory" -Processors "$HyperV_MaxProcessors"
	[int]$i = $i
}
until ($i -eq $NumberOfManagementServers)
#SQL Server
Add-LabMachineDefinition -Name MSSQL-2012R2 -Roles $SQLServer2012Role -NetworkAdapter $SQL2012R2NetAdapter -Memory "$HyperV_HighestMemory" -MinMemory "$HyperV_MaxMemory" -MaxMemory "$HyperV_HighestMemory" -Processors "$HyperV_MaxProcessors"
#IIS front-end server
Add-LabMachineDefinition -Name IIS-2012R2 -NetworkAdapter $IISAgentNetAdapter
if ($DeployRHEL79)
{
...
# Copy SQL Server Install Media to Local C:\ Drive
$SQLServers = Get-LabVM | Where-Object -FilterScript { $_.Roles -in "SQLServer2012" }
foreach ($CurrentSQLServer in $SQLServers.Name)
{
	$Drive = Mount-LabIsoImage -ComputerName $CurrentSQLServer -IsoPath $labSources\$SQLServerISO -PassThru
	if ($SQLProductKey)
	{
		Invoke-LabCommand -ActivityName 'Upgrading SQL 2012R2 from Evaluation to Full Version.' -ComputerName $CurrentSQLServer -ScriptBlock {
	...
	Invoke-LabCommand -ActivityName 'Copying SQL Install Files locally' -ComputerName $CurrentSQLServer -ScriptBlock {
		$Sxs = New-Item -Path $SQLSetupLocalFolder -ItemType Directory -Force
		Copy-Item -Path "$($Drive.DriveLetter)\*" -Destination $SQLSetupLocalFolder -Recurse -Force
	} -Variable (Get-Variable -Name Drive, SQLSetupLocalFolder)
	Dismount-LabIsoImage -ComputerName $CurrentSQLServer
	Restart-LabVM -ComputerName $CurrentSQLServer -Wait
}
...
Set-DNSClientServerAddress -InterfaceIndex $network_adapters.ifIndex[0] -ServerAddresses $DC2012R2IPv4Address -ErrorAction SilentlyContinue
...
<#
#Downloading SQL Server 2012R2 CU8 (or later)
$SQLServer2012LatestCU = Get-Item $labSources\SoftwarePackages\SQLServer2012-KB*-x64.exe | Sort Name | Select -Last 1
if (!$SQLServer2012LatestCU)
{
	$SQLServer2012LatestCU = Get-LabInternetFile -Uri $SQLServer2012LatestCUURI -Path $labSources\SoftwarePackages -PassThru
}
#Installing SQL Server 2012R2 CU8 (or later)
Install-LabSoftwarePackage -ComputerName MSSQL-2012R2 -Path $SQLServer2012LatestCU.FullName -CommandLine " /QUIET /IACCEPTSQLSERVERLICENSETERMS /ACTION=PATCH /ALLINSTANCES" #-AsJob
#>
#Get-Job -Name 'Installation of*' | Wait-Job | Out-Null
$i = 0
$SCOMServers = Get-LabVM | Where-Object -FilterScript { $_.Name -like "MS*-2012R2*" -and $_.Roles -notin "SQLServer2012" }
...
Invoke-LabCommand -ActivityName 'Domain Naming Service (DNS) & Active Directory Users / Groups (AD) Setup on Domain Controller (DC)' -ComputerName DC-2012R2 -ScriptBlock {
...
$LocalAdminServers = Get-LabVM | Where-Object -FilterScript { $_.Name -like "MS*-2012R2*" -and $_.Roles -notin "SQLServer2012" }
$LocalAdminServers += 'MSSQL-2012R2', 'IIS-2012R2'
Invoke-LabCommand -ActivityName 'Adding the SCOM Admins AD Group to the local Administrators Group' -ComputerName $LocalAdminServers -ScriptBlock {
	Write-Host "Adding Local Admin Permissions on: $env:COMPUTERNAME"
	#Add New Local User
	#net user bdrumm Password1 /add
	net localgroup administrators "$NetBiosDomainName\$SCOMAdmins" /add
...
Invoke-LabCommand -UseLocalCredential -ActivityName "Disabling IE ESC" -ComputerName $WindowsMachines -ScriptBlock {
	...
	try
	{
		Set-NetConnectionProfile -InterfaceAlias * -NetworkCategory "Private" -Confirm:$false -ErrorAction Stop
	}
	catch { }
	Write-Verbose "Setting the Network Profile to Private to allow the servers to communicate during installation."
} -Variable (Get-Variable -Name UseDefaultSwitch, labName)

Invoke-LabCommand -ActivityName 'Installing IIS, ASP and ASP.NET 4.5+' -ComputerName IIS-2012R2 -ScriptBlock {
...
try
{
	Invoke-WebRequest -Method GET -Uri 'https://www.powershellgallery.com/api/v2/package/PSWindowsUpdate' -ErrorAction Stop | Out-Null
	...
	try
	{
		...
		Expand-Archive $labSources\SoftwarePackages\windowsupdate_powershell.zip -DestinationPath $labSources\SoftwarePackages\WindowsUpdate_Powershell -ErrorAction Stop
	}
...
catch { Write-Verbose 'Experienced an error when attempting to Download / Install the Windows Update Powershell.' }

Invoke-LabCommand -ActivityName "Adding users to the SQL SysAdmin Group / Modifying Windows Firewall" -ComputerName MSSQL-2012R2 -ScriptBlock {
	
	$sql = @"
create login [$NetBiosDomainName\$SQLUser] from windows;
-- For newer versions of SQL Server:
ALTER SERVER ROLE [sysadmin] ADD MEMBER [$NetBiosDomainName\$SQLUser];

GO
"@
	
	invoke-sqlcmd -ServerInstance MSSQL-2012R2\SCOM2012R2 -Query $sql
	
	if ($CustomDomainAdmin)
	{
		$sql = @"
create login [$NetBiosDomainName\$CustomDomainAdmin] from windows;
-- For newer versions of SQL Server:
ALTER SERVER ROLE [sysadmin] ADD MEMBER [$NetBiosDomainName\$CustomDomainAdmin];
...
"@
	...
	$sql = @"
create login [$NetBiosDomainName\$Logon] from windows;
-- For newer versions of SQL Server:
ALTER SERVER ROLE [sysadmin] ADD MEMBER [$NetBiosDomainName\$Logon];
...
$SystemCLRTypesForSQLServer2012x64 = Get-LabInternetFile -Uri $SystemCLRTypesForSQLServer2012x64URI -Path $labSources\SoftwarePackages -File SQLSysClrTypes2012.msi -PassThru
Install-LabSoftwarePackage -ComputerName $SCOMServers -Path $SystemCLRTypesForSQLServer2012x64.FullName -CommandLine "/qn  /L* $(Join-Path -Path $env:SystemDrive -ChildPath $($SystemCLRTypesForSQLServer2012x64.FileName + ".log")) /norestart ALLUSERS=2"

$ReportViewer2012Runtime = Get-LabInternetFile -Uri $ReportViewer2012RuntimeURI -Path $labSources\SoftwarePackages -File ReportViewer2012.msi -PassThru
Install-LabSoftwarePackage -ComputerName $SCOMServers -Path $ReportViewer2012Runtime.FullName -CommandLine "/qn /L* $(Join-Path -Path $env:SystemDrive -ChildPath $($ReportViewer2012Runtime.FileName + ".log")) /norestart ALLUSERS=2"

#Get-Job -Name 'Installation of*' | Wait-Job | Out-Null
foreach ($CurrentSCOMServer in $SCOMServers.Name)
{
	$Drive = Mount-LabIsoImage -ComputerName $CurrentSCOMServer -IsoPath $labSources\$SCOM2012R2_Location -PassThru
	Invoke-LabCommand -ActivityName 'Copying SCOM Install Files locally' -ComputerName $CurrentSCOMServer -ScriptBlock {
		$Sxs = New-Item -Path $SCOMSetupLocalFolder -ItemType Directory -Force
		Copy-Item -Path "$($Drive.DriveLetter)\*" -Destination $SCOMSetupLocalFolder -Recurse -Force
	} -Variable (Get-Variable -Name Drive, SCOMSetupLocalFolder)
	Dismount-LabIsoImage -ComputerName $CurrentSCOMServer
}
Invoke-LabCommand -ActivityName 'Installing the Operations Manager Management Server on the RMSE (MS01-2012R2)' -ComputerName ($SCOMServers | Where{ $_.Name -like '*01-*' }) -ScriptBlock {
	
	#Setting up SCOM Management Server
	$ArgumentList = @(
		"/silent /install /components:OMServer /ManagementGroupName:$SCOMMgmtGroup /SqlServerInstance:MSSQL-2012R2\$LabName",
		"/DatabaseName:OperationsManager /DWSqlServerInstance:MSSQL-2012R2\$LabName /DWDatabaseName:OperationsManagerDW /ActionAccountUser:$NetBiosDomainName\$SCOMServerAction",
	...
	Write-Host "Done Installing Management Server on $env:COMPUTERNAME"
	if ($SCOMProductKey -match "^\w{5}-\w{5}-\w{5}-\w{5}-\w{5}$")
	{
		sleep 30
		#Importing the OperationsManager module by specifying the full folder path
		Import-Module "${env:ProgramFiles}\Microsoft System Center 2012 R2\Operations Manager\Powershell\OperationsManager"
		...
		if ((Get-SCOMManagementGroup).SKUForLicense -ne 'Retail')
		{
			...
			Set-SCOMLicense -ProductId $SCOMProductKey -Credential:$Cred -Confirm:$false
...
if ($NumberOfManagementServers -gt '1')
{
	sleep 30
	Invoke-LabCommand -ActivityName 'Installing the Operations Manager Management Server on the Rest of the Management Servers' -ComputerName ($SCOMServers | Where{ $_.Name -notlike '*01-*' }) -ScriptBlock {
...
sleep 30
Invoke-LabCommand -ActivityName 'Installing the Operations Manager Console' -ComputerName $SCOMServers -ScriptBlock {
...
sleep 15
Invoke-LabCommand -ActivityName 'Installing the Operations Manager Web Console' -ComputerName MS01-2012R2 -ScriptBlock {
...
Install-LabSoftwarePackage -ComputerName MSSQL-2012R2 -Path $SQLServer2017ReportingServices.FullName -CommandLine " /quiet /IAcceptLicenseTerms /Edition=Eval"
#Get-Job -Name 'Installation of*' | Wait-Job | Out-Null
sleep 1
Invoke-LabCommand -ActivityName 'Configuring Report Server on SQL Server' -ComputerName MSSQL-2012R2 -ScriptBlock {
	...
	If (! $configset.IsInitialized)
	{
		...
		# For SSRS 2012R2-2017 only, older versions have a different name
...
#>
Install-LabSoftwarePackage -ComputerName MSSQL-2012R2 -Path "$labSources\$SCOM2012R2_Location" -CommandLine "/dir=`"$SCOMSetupLocalFolder`" `"/silent`"" -ErrorAction Stop
Invoke-LabCommand -ActivityName 'Installing the Operations Manager Reporting Services' -ComputerName MSSQL-2012R2 -ScriptBlock {
	#Setting up SCOM
	$ArgumentList = @(
		"/silent /install /components:OMReporting /ManagementServer:MS01-2012R2 /SRSInstance:$env:ComputerName\SSRS",
...
Dismount-LabIsoImage -ComputerName MSSQL-2012R2


Invoke-LabCommand -ActivityName 'Cleanup on SQL Server' -ComputerName MSSQL-2012R2 -ScriptBlock {
...
Install-LabSoftwarePackage -ComputerName MS01-2012R2 -Path $SCOMWSManagementPack.FullName -CommandLine "-quiet"
#Install-LabSoftwarePackage -ComputerName MS01-2012R2 -Path $SCOMWS2012R2andWS2012R2ManagementPack.FullName -CommandLine "-quiet"
Install-LabSoftwarePackage -ComputerName MS01-2012R2 -Path $SCOMIISManagementPack.FullName -CommandLine "-quiet"
Install-LabSoftwarePackage -ComputerName MS01-2012R2 -Path $SCOMNETAPMManagementPack.FullName -CommandLine "-quiet"
#Get-Job -Name 'Installation of*' | Wait-Job | Out-Null

Invoke-LabCommand -ActivityName 'Installing Management Packs' -ComputerName MS01-2012R2 -ScriptBlock {
	...
	#'Microsoft.SystemCenter.ApplicationInsights', 'Microsoft.Windows.InternetInformationServices.2012R2', 'Microsoft.Windows.InternetInformationServices.CommonLibrary','Microsoft.Windows.Server.2012R2.Discovery', 'Microsoft.Windows.Server.Library' | Get-SCOMManagementPack | Remove-SCOMManagementPack
	#Installing Windows Server Management Pack prior IIS
	#Importing the OperationsManager module by specifying the full folder path
	Import-Module "${env:ProgramFiles}\Microsoft System Center 2012 R2\Operations Manager\Powershell\OperationsManager"
	& "$env:ProgramFiles\Microsoft System Center 2012 R2\Operations Manager\Powershell\OperationsManager\Functions.ps1"
	& "$env:ProgramFiles\Microsoft System Center 2012 R2\Operations Manager\Powershell\OperationsManager\Startup.ps1"
	...
	Get-ChildItem -Path "${env:ProgramFiles(x86)}\System Center Management Packs\" -File -Filter *.mp? -Recurse | Where-Object -FilterScript { $_.BaseName -in 'Microsoft.Windows.Server.2012R2.Discovery' } | Import-SCOMManagementPack
	...
	Get-ChildItem -Path "${env:ProgramFiles(x86)}\System Center Management Packs\" -File -Filter *.mp? -Recurse | Where-Object -FilterScript { $_.BaseName -in 'Microsoft.Windows.Server.2012R2.Monitoring' } | Import-SCOMManagementPack
	...
	Get-ChildItem -Path "${env:ProgramFiles(x86)}\System Center Management Packs\" -File -Filter *.mp? -Recurse | Where-Object -FilterScript { $_.BaseName -in 'Microsoft.Windows.InternetInformationServices.2012R2' } | Import-SCOMManagementPack
...
Invoke-LabCommand -ActivityName 'Installing SCOM Agent(s)' -ComputerName ($SCOMServers | Where{ $_.Name -like '*01-*' }) -ScriptBlock {
	foreach ($machine in $ToBecomeAgents)
	{
		Install-SCOMAgent -PrimaryManagementServer $(Get-SCOMManagementServer -Name "MS01-2012R2.$FQDNDomainName") -DNSHostName $machine
	}
	$DomainControllers = "DC-2012R2.$FQDNDomainName"
	<#
	function Time-Stamp
	{
...
Invoke-LabCommand -ActivityName 'Installing SCOM Agent to Domain Controller(s)' -ComputerName ($ToBecomeAgents | Where { $_.Name -like "DC*" }) -ScriptBlock {
	...
	start-process -FilePath "$env:windir\System32\msiexec.exe" -ArgumentList "/x $path_to_install /qn /l*v C:\\AgentUninstall.log" -Wait -ErrorAction SilentlyContinue
	Sleep 1
	$args = "USE_SETTINGS_FROM_AD=0 USE_MANUALLY_SPECIFIED_SETTINGS=1 MANAGEMENT_GROUP=$LabName MANAGEMENT_SERVER_DNS=MS01-2012R2.$FQDNDomainName MANAGEMENT_SERVER_AD_NAME=MS01-2012R2.$FQDNDomainName SECURE_PORT=5723 ACTIONS_USE_COMPUTER_ACCOUNT=1 AcceptEndUserLicenseAgreement=1"
	...
	start-process -FilePath "$env:windir\System32\msiexec.exe" -ArgumentList $args -Wait -ErrorAction SilentlyContinue
	sleep 1
...
} -Variable (Get-Variable -Name FQDNDomainName)
...
if ($UseDefaultSwitch)
{
	...
	Get-VM -Name 'MSSQL-2012R2' | Remove-VMNetworkAdapter -Name 'Default Switch' -ErrorAction SilentlyContinue
}
