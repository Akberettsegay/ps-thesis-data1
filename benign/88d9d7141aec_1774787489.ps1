<# 
.DESCRIPTION 
    Disable user profile photo permission in the default policy
.NOTES 
    Vertsion:   1.0
    Author: Hugo Santos
...
Try{
    Write-Log -Message "Disabling User Profile Photo Permission in the Default policy" -Type "Information"
    Set-OwaMailboxPolicy -Identity OwaMailboxPolicy -Default -SetPhotoEnabled $False
    Write-Log -Message "Disabled User Profile Photo" -Type "Success"
}Catch{
    Write-Host "`n`t$($_.InvocationInfo.InvocationName) [Line:$($_.InvocationInfo.ScriptLineNumber)]: $($_.Exception.Message)" -ForegroundColor Yellow
    Write-Log -Message "$($_.InvocationInfo.InvocationName) [Line:$($_.InvocationInfo.ScriptLineNumber)]: $($_.Exception.Message)" -Type "Error"
    Write-Log -Message "Disabling User Profile Photo failed" -Type "Error"
}
