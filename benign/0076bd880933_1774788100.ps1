...
try {
    ...
    Write-Error $_
    Send-TelegramMessage -Message '\\\ Project pwshCloudCommands - PubXMLMonitor Error retrieving object from S3'
    return
}

if ($null -eq $objInfo) {
    Write-Warning -Message 'No object returned from S3'
    Send-TelegramMessage -Message '\\\ Project pwshCloudCommands - PubXMLMonitor The S3 object was not found'
}
...
try {
    ...
    Send-TelegramMessage -Message '\\\ Project pwshCloudCommands - PubXMLMonitor Error sending metric data to CloudWatch'
}
