...
Start-PodeServer -Threads 1 -ScriptBlock {
    ...
    if ((Get-PodeConfig).Protocol -eq 'Https') {
        if ((Get-PodeConfig).SelfSignedCertificate) {
            ...
            Add-PodeEndpoint -Address (Get-PodeConfig).Address -Port ((Get-PodeConfig).RestFulPort + 1) -Protocol Https -Certificate $Certificate -CertificateKey $CertificateKey -CertificatePassword (Get-PodeConfig).CertificatePassword -Name 'endpoint_v3.1'
        }
    }
    else {
...
'@
    ...
    Select-PodeOADefinition -Tag 'v3.0.3', 'v3.1'  -Scriptblock {
        ...
        New-PodeOAContentMediaType -ContentType 'application/json', 'application/xml' -Content 'Pet' | Add-PodeOAComponentRequestBody -Name 'Pet' -Description 'Pet object that needs to be added to the store'
