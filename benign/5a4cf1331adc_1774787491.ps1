<# 
.DESCRIPTION 
    Export all mailboxes with forwarding configured
.NOTES 
    Vertsion:   1.0
    Author: Hugo Santos
...
Try {
    Write-log -Message 'Getting mailbox with forward' -Type 'Information'
    $Users = Get-Mailbox -RecipientType UserMailbox -ResultSize Unlimited
    Foreach ($User in $Users) {
        $Mailbox_Forwarding = Get-Mailbox -Identity $User.PrimarySmtpAddress | Select-Object UserPrincipalName, PrimarySmtpAddress, distinguishedName, DeliverToMailboxAndForward, ForwardingAddress, ForwardingSmtpAddress
        if ($Mailbox_Forwarding.DeliverToMailboxAndForward -or $Mailbox_Forwarding.ForwardingAddress -or $Mailbox_Forwarding.ForwardingSmtpAddress) {
            $Export_Forwarding = [PSCustomObject][Ordered]@{
                'UserPrincipalName'     = $Mailbox_Forwarding.UserPrincipalName
                'DeliverAndForward'     = $Mailbox_Forwarding.DeliverToMailboxAndForward
                'ForwardingAddress'     = $Mailbox_Forwarding.ForwardingAddress
                'ForwardingSmtpAddress' = $Mailbox_Forwarding.ForwardingSmtpAddress
            }
            $Export_Forwarding | Export-Csv -Path 'C:\Temp\Export\Fowrding_Configured.csv' -Delimiter ',' -Encoding UTF8 -NoClobber -NoTypeInformation -Append -Force
        }
    }
    Write-log -Message 'Exported all information' -Type 'Sucess'
} Catch {
    Write-Host "`n`t$($_.InvocationInfo.InvocationName) [Line:$($_.InvocationInfo.ScriptLineNumber)]: $($_.Exception.Message)" -ForegroundColor Yellow
    Write-Log -Message "$($_.InvocationInfo.InvocationName) [Line:$($_.InvocationInfo.ScriptLineNumber)]: $($_.Exception.Message)" -Type 'Error'
