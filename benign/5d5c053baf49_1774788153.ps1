...
###############################################################################################################
#                                                                                                             #  
# This is a special version of ACLight especially made for "HotSpot" research and for "PreCog" detection tool #
#                                                                                                             #
#                       The updated version of ACLight is in the offical GitHub page:                         #
#                                                                                                             #
...
version 4: 25.12.17 -> a special version of ACLight especially made for "HotSpot" research and for "PreCog" detection tool
The updtaed version is in:
https://github.com/cyberark/ACLight
...
THE RESULTS FILES:
* This specific ACLight version of the scan will not produce all of those results files, only the results files that PreCog Needs.
...
function Write-LayersInfo {
    [CmdletBinding()]
        ...
        $layersOutputArray.Add('Accounts are only written once, but acctualy they can also be members of other groups - check the detailed results in the "Final report"')
        $domainArray |  Where-Object {$_} | ForEach-Object {
            $layersOutputArray.Add("`n#################################################################`n")
                    ...
                    if (-not ($groupLine.AccountGroup -eq $groupLine.AccountName)) {
                        foreach ($groupMember in $groupLine) {
                            if ($layer -eq $groupMember.layer) {
                                ...
                                if (-not (($layersOutputArray.contains($gap + $groupMember.AccountName  + " - group") -or ($layersOutputArray.contains($gap + $gap + $groupMember.AccountName))))) {
    ...
    if ($PSVersionTable.PSVersion.Major -ge 3){
        ...
        foreach ($Domain in $DomainList){
            ...
            while ($finishDomainScan -eq $False) {
                ...
                # to do:  to add scan for object that could not be read due to lack of read permission - could be suspicious

                $counter++
                if (-not $finishedDomain){
                    Write-Output "Scanning ACLs Analysis - Layer $domainLayerCounter"
                    Start-domainACLsAnalysis -Full $Full -exportCsvFile $exportCsvFile -Domain $Domain -entitySIDList $newSIDtoScan -multiLayered 
                }
                else {
                    Write-Output "Analysis in progress..."
                    Invoke-ACLcsvFileAnalysis -Full $Full -exportCsvFile $exportCsvFile -Domain $Domain
                    # open new process for each domain:
                    #The scan will automatically scan all the domain in a parallel time - it's much more time efficient
                    #$processPointer = start-process powershell.exe -WorkingDirectory $ScriptRoot -argument "-noprofile -ExecutionPolicy Bypass Import-Module '.\MultiLayered-ACLight.psm1' -force ; Invoke-ACLcsvFileAnalysis -Full $Full -exportCsvFile $exportCsvFile -Domain $Domain"
                    #$processPointersList += $processPointer
        ...
        # this is the orignal list from ACLight:
        ...
        ###################
        # to do - to add check if there are object that you cannot read
        ###################

        Write-Output "`n------------ FINISHED ------------"
        Write-host "`nDiscovered $numberAccounts privileged accounts" -ForegroundColor Yellow
        Write-Output "Check the list of the accounts with extra permissions:`n$exportListFile"
...
#>
    ...
    try{
        ...
        Get-ObjectACL @PSBoundParameters | ForEach-Object {
            ...
            #######
            # the following filter is like the ACL privilege filter in BloodHound open source project:
