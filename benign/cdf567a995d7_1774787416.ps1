...
function Select-MsalClientApplication {
    ...
    switch -Wildcard ($PSCmdlet.ParameterSetName) {
        'PublicClient*' {
            [Microsoft.Identity.Client.IPublicClientApplication] $ClientApplication = $PublicClientApplications | Where-Object { $_.AppConfig.ClientId -eq $NewClientApplication.AppConfig.ClientId -and $_.AppConfig.RedirectUri -eq $NewClientApplication.AppConfig.RedirectUri -and $_.AppConfig.IsBrokerEnabled -eq $NewClientApplication.AppConfig.IsBrokerEnabled } | Select-Object -Last 1
            break
        }
