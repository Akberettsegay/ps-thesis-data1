...
[CmdletBinding(DefaultParameterSetName="Help")]
    Param (
        ...
        [parameter(ParameterSetName="Upgrade")] [switch]$Takeover, ## Optional Takeover existing installation, covert to passphrase and move to location assigned by Customer UID
        ...
        [parameter(ParameterSetName="Upgrade")] 
            [parameter(ParameterSetName="Remove")] [Alias("Store")] [switch]$Copy,  ## Store credentials from existing installation
    ...
    Write-Output ""
    Write-Output "  N-able Backup Installer"
    ...
    Function Autodeploy-Passphrase {
        ...
        if (((Test-Path $BMConfig -PathType leaf) -eq $false) -or ($Force)) {
            ...
            Write-Output "  Set Bandwidth  : $($SetBandwidth.IsPresent)"
            Write-Output ""
    ...
    Function Set-Archive {
        Write-Output "  Setting Archive Settings`n"
        start-sleep 60
        & "$clienttool" control.archiving.add -name "#Monthly - Archive#" -days-of-month 1
        & "$clienttool" control.archiving.list
    }
    Function Set-Bandwidth {
        ...
        [String]$Start = "8:00"                                                           ## Throttle Start
        [String]$Stop = "17:00"                                                           ## Throttle Stop
        [String]$download = "-1"                                                          ## Throttle Download (-1 = Unlimited)
        [String]$upload = "5120"                                                          ## Throttle Upload

        $headers = New-Object "System.Collections.Generic.Dictionary[[String],[String]]"
        $headers.Add("Content-Type", "application/json")
        $body = "{`"id`":`"jsonrpc`",`"jsonrpc`": `"2.0`",`"method`":`"SaveBandwidthOptions`",`"params`": {`"limitBandWidth`":$Active ,`"turnOnAt`":`"$Start`",`"turnOffAt`":`"$Stop`",`"maxUploadSpeed`":$upload,`"maxDownloadSpeed`":$download,`"dataThroughputUnits`":`"KBits`",`"unlimitedDays`":[`"Saturday`",`"Sunday`"],`"pluginsToCancel`":[]} }"
