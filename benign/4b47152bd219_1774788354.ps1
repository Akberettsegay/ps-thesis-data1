...
$LabName = 'SCOM2022Preview' # (Default: SCOM2022Preview)
...
$SCOMSetupLocalFolder = 'C:\System Center Operations Manager' #Where you want to extract the SCOM Installation when installing the SCOM Components to the Management Server. (Default: C:\System Center Operations Manager)
$WindowsOperatingSystem = 'Windows Server 2022 Datacenter Evaluation (Desktop Experience)' #Set the Operating System Type here. Grab the OS with this command: Get-LabAvailableOperatingSystem
...
# Hostname | Role
# DC-2022 : Domain Controller
# MS01-2022 : System Center Operations Manager 2022 - Management Server
# MSSQL-2022 : SQL Server 2019
# IIS-2022 : IIS Windows Agent
# RHEL7-9 : Redhat 7.9

##############################################
########### Network / IP Address #############
##############################################
$NetworkID = '192.168.4.0/24' # (Default: 192.168.4.0/24)
$DC2022IPv4Address = '192.168.4.1' # (Default: 192.168.4.1)
$GatewayIPv4Address = '192.168.4.2' # Hyper-V Host IP (Default: 192.168.4.2)
$SQL2019IPv4Address = '192.168.4.11' #SQL 2019 Server (Default: 192.168.4.11)
$IIS_IPv4Address = '192.168.4.21' #SCOM Web Console (Default: 192.168.4.21)
$SCOM2022MSIPv4Address = '192.168.4.31' # (Default: 192.168.4.31)
#Redhat 7.9 - if you dont want this deployed, set $DeployRHEL79 = $false.
$DeployRHEL79 = $false # (Default: $false)
$RHEL79IPv4Address = '192.168.4.58' # (Default: 192.168.4.58)
...
############### Product Keys #################
##############################################
#Currently written to only license DataCenter Evaluation version: https://www.microsoft.com/en-us/evalcenter/evaluate-windows-server-2022
...
$SCOMMgmtGroup = 'SCOM-2022-MG' # SCOM management group name (Default: SCOM-2022-MG)
#SQL Accounts
$SQLUser = 'SQLUser' # User Name with admin rights on SQL Server (Default: SQLUser)
#############################################
...
#$GatewayIPv4Address = (Get-NetIPInterface | Where{ $_.InterfaceAlias -like '*SCOM2022*' } | Where{ $_.AddressFamily -eq 'IPv4' } | Get-NetIPAddress).IPAddress

$IISAgentNetAdapter = @()
$IISAgentNetAdapter += New-LabNetworkAdapterDefinition -VirtualSwitch $LabName -Ipv4Address $IIS_IPv4Address -Ipv4Gateway $GatewayIPv4Address -Ipv4DNSServers $DC2022IPv4Address -RegisterInDNS:$true
if ($UseDefaultSwitch)
{
	$IISAgentNetAdapter += New-LabNetworkAdapterDefinition -VirtualSwitch 'Default Switch' -UseDhcp
}
$SQL2019NetAdapter = @()
$SQL2019NetAdapter += New-LabNetworkAdapterDefinition -VirtualSwitch $LabName -Ipv4Address $SQL2019IPv4Address -Ipv4Gateway $GatewayIPv4Address -Ipv4DNSServers $DC2022IPv4Address -RegisterInDNS:$true
if ($UseDefaultSwitch)
{
...
do
{
	...
	$i++
	[string]$i = "{0:D2}" -f $i
	$SCOM2022MSNetAdapter["var$i"] = @()
	$MSIPv4Address = ((($SCOM2022MSIPv4Address).Split(".") | Select -First 3) -join '.') + "." + ([int](($SCOM2022MSIPv4Address).Split(".") | Select -Last 1) + $i)
	$SCOM2022MSNetAdapter["var$i"] += New-LabNetworkAdapterDefinition -VirtualSwitch $LabName -Ipv4Address $MSIPv4Address -Ipv4Gateway $GatewayIPv4Address -Ipv4DNSServers $DC2022IPv4Address -RegisterInDNS:$true
	if ($UseDefaultSwitch)
	{
		#Adding an Internet Connection on the Management Server
		$SCOM2022MSNetAdapter["var$i"] += New-LabNetworkAdapterDefinition -VirtualSwitch 'Default Switch' -UseDhcp
	}
	[int]$i = $i
}
...
{
	$RHEL79NetAdapter = @()
	$RHEL79NetAdapter += New-LabNetworkAdapterDefinition -VirtualSwitch $LabName -Ipv4Address $RHEL79IPv4Address -Ipv4Gateway $GatewayIPv4Address -Ipv4DNSServers $DC2022IPv4Address -RegisterInDNS:$true
	if ($UseDefaultSwitch)
	{
...
Add-LabMachineDefinition -Name DC-2022 -Roles RootDC -IpAddress $DC2022IPv4Address -Gateway $GatewayIPv4Address -DnsServer1 $DC2022IPv4Address
#SCOM server
$i = 0
do
{
	...
	$i++
	[string]$i = "{0:D2}" -f $i
	Add-LabMachineDefinition -Name "MS$i`-2022" -NetworkAdapter $SCOM2022MSNetAdapter["var$i"] -Memory "$HyperV_HighestMemory" -MinMemory "$HyperV_MinMemory" -MaxMemory "$HyperV_HighestMemory" -Processors "$HyperV_MaxProcessors"
	[int]$i = $i
}
until ($i -eq $NumberOfManagementServers)
#SQL Server
Add-LabMachineDefinition -Name MSSQL-2022 -Roles $SQLServer2019Role -NetworkAdapter $SQL2019NetAdapter -Memory "$HyperV_HighestMemory" -MinMemory "$HyperV_MinMemory" -MaxMemory "$HyperV_HighestMemory" -Processors "$HyperV_MaxProcessors"
#IIS front-end server
Add-LabMachineDefinition -Name IIS-2022 -NetworkAdapter $IISAgentNetAdapter
if ($DeployRHEL79)
{
...
if ($SQLProductKey)
{
	Restart-LabVM -ComputerName MSSQL-2022 -Wait
	$Drive = Mount-LabIsoImage -ComputerName MSSQL-2022 -IsoPath $labSources\$SQLServerISO -PassThru
	Invoke-LabCommand -ActivityName 'Upgrading SQL 2019 from Evaluation to Full Version.' -ComputerName MSSQL-2022 -ScriptBlock {
	...
	Dismount-LabIsoImage -ComputerName MSSQL-2022
	Restart-LabVM -ComputerName MSSQL-2022 -Wait
}
...
Set-DNSClientServerAddress -InterfaceIndex $network_adapters.ifIndex[0] -ServerAddresses $DC2022IPv4Address -ErrorAction SilentlyContinue
...
$SQLServer2019LatestCU = Get-Item $labSources\SoftwarePackages\SQLServer2019-KB*-x64.exe | Sort Name | Select -Last 1
if (!$SQLServer2019LatestCU)
{
...
Install-LabSoftwarePackage -ComputerName MSSQL-2022 -Path $SQLServer2019LatestCU.FullName -CommandLine " /QUIET /IACCEPTSQLSERVERLICENSETERMS /ACTION=PATCH /ALLINSTANCES" #-AsJob
#Get-Job -Name 'Installation of*' | Wait-Job | Out-Null
$i = 0
$SCOMServers = Get-LabVM | Where-Object -FilterScript { $_.Name -like "MS*-2022*" -and $_.Roles -notin "SQLServer2019" }
...
Invoke-LabCommand -ActivityName 'Domain Naming Service (DNS) & Active Directory Users / Groups (AD) Setup on Domain Controller (DC)' -ComputerName DC-2022 -ScriptBlock {
...
$LocalAdminServers = Get-LabVM | Where-Object -FilterScript { $_.Name -like "MS*-2022*" -and $_.Roles -notin "SQLServer2019" }
$LocalAdminServers += 'MSSQL-2022', 'IIS-2022'
...
Invoke-LabCommand -ActivityName 'Installing IIS, ASP and ASP.NET 4.5+' -ComputerName IIS-2022 -ScriptBlock {
...
$sqlserverNupkg = Get-Item $labSources\SoftwarePackages\sqlserver_powershell.nupkg -ErrorAction SilentlyContinue
if (!$sqlserverNupkg)
{
...
try
{
	Rename-Item -Path $sqlserverNupkg.FullName $labSources\SoftwarePackages\sqlserver_powershell.zip -Force
	Expand-Archive $labSources\SoftwarePackages\sqlserver_powershell.zip -DestinationPath $labSources\SoftwarePackages\SqlServer_Powershell -ErrorAction Stop
}
...
$files_to_remove | Remove-Item -Recurse -Confirm:$false -ErrorAction SilentlyContinue

Copy-LabFileItem -ComputerName MSSQL-2022 $labSources\SoftwarePackages\SqlServer_Powershell
...
Invoke-LabCommand -ActivityName 'Installing the latest SQL Server Powershell Module' -ComputerName MSSQL-2022 -ScriptBlock {
...
Invoke-LabCommand -ActivityName "Adding users to the SQL SysAdmin Group / Modifying Windows Firewall" -ComputerName MSSQL-2022 -ScriptBlock {
...
Install-LabSoftwarePackage -ComputerName $SCOMServers -Path $SystemCLRTypesForSQLServer2014x64.FullName -CommandLine "/qn  /L* $(Join-Path -Path $env:SystemDrive -ChildPath $($SystemCLRTypesForSQLServer2014x64.FileName + ".log")) /norestart ALLUSERS=2"

$ReportViewer2015Runtime = Get-LabInternetFile -Uri $ReportViewer2015RuntimeURI -Path $labSources\SoftwarePackages -PassThru
Install-LabSoftwarePackage -ComputerName $SCOMServers -Path $ReportViewer2015Runtime.FullName -CommandLine "/qn /L* $(Join-Path -Path $env:SystemDrive -ChildPath $($ReportViewer2015Runtime.FileName + ".log")) /norestart ALLUSERS=2"

#Get-Job -Name 'Installation of*' | Wait-Job | Out-Null
$LabSourcesLocation = Get-LabSourcesLocation
Install-LabSoftwarePackage -ComputerName $SCOMServers -Path "$LabSourcesLocation\$SCOM2022_Location" -CommandLine "/dir=`"$SCOMSetupLocalFolder`" `"/silent`"" -ErrorAction Stop

Invoke-LabCommand -ActivityName 'Installing the Operations Manager Management Server on the RMSE (MS01-2022)' -ComputerName ($SCOMServers | Where{ $_.Name -like '*01-*' }) -ScriptBlock {
	
	#Setting up SCOM Management Server
	$ArgumentList = @(
		"/silent /install /components:OMServer /ManagementGroupName:$SCOMMgmtGroup /SqlServerInstance:MSSQL-2022\$LabName",
		"/DatabaseName:OperationsManager /DWSqlServerInstance:MSSQL-2022\$LabName /DWDatabaseName:OperationsManagerDW /ActionAccountUser:$NetBiosDomainName\$SCOMServerAction",
	...
	if ($SCOMProductKey -match "^\w{5}-\w{5}-\w{5}-\w{5}-\w{5}$")
	{
		...
		if ((Get-SCOMManagementGroup).SKUForLicense -ne 'Retail')
		{
			...
			Set-SCOMLicense -ProductId $SCOMProductKey -ManagementServer $env:COMPUTERNAME -Credentials:$Cred -Confirm:$false
...
Invoke-LabCommand -ActivityName 'Installing the Operations Manager Console' -ComputerName $SCOMServers -ScriptBlock {
	...
	if ($SCOMProductKey -match "^\w{5}-\w{5}-\w{5}-\w{5}-\w{5}$")
	{
		...
		if ((Get-SCOMManagementGroup).SKUForLicense -ne 'Retail')
		{
			$Cred = New-Object System.Management.Automation.PSCredential ($(whoami), $SecurePassword)
			#To properly license SCOM, install the product key using the following cmdlet: 
			Set-SCOMLicense -ProductId $SCOMProductKey -ManagementServer $((Get-SCOMManagementServer)[0]) -Credential:$Cred -Confirm:$false
			#(Re)Starting the 'System Center Data Access Service'is mandatory to take effect
			Restart-Service healthservice, omsdk, cshost -ErrorAction SilentlyContinue #-Force
		}
	}
...
sleep 15
Invoke-LabCommand -ActivityName 'Installing the Operations Manager Web Console' -ComputerName MS01-2022 -ScriptBlock {
...
Install-LabSoftwarePackage -ComputerName MSSQL-2022 -Path $SQLServer2019ReportingServices.FullName -CommandLine " /quiet /IAcceptLicenseTerms /Edition=Eval"
#Get-Job -Name 'Installation of*' | Wait-Job | Out-Null
sleep 1
Invoke-LabCommand -ActivityName 'Configuring Report Server on SQL Server' -ComputerName MSSQL-2022 -ScriptBlock {
...
Install-LabSoftwarePackage -ComputerName MSSQL-2022 -Path "$LabSourcesLocation\$SCOM2022_Location" -CommandLine "/dir=`"$SCOMSetupLocalFolder`" `"/silent`"" -ErrorAction Stop
Invoke-LabCommand -ActivityName 'Installing the Operations Manager Reporting Services' -ComputerName MSSQL-2022 -ScriptBlock {
	#Setting up SCOM
	$ArgumentList = @(
		"/silent /install /components:OMReporting /ManagementServer:MS01-2022 /SRSInstance:$env:ComputerName\SSRS",
...
Invoke-LabCommand -ActivityName 'Cleanup on SQL Server' -ComputerName MSSQL-2022 -ScriptBlock {
...
#Install-LabSoftwarePackage -ComputerName MS01-2022 -Path $SCOMWS2016andWS2019ManagementPack.FullName -CommandLine "-quiet"
#Install-LabSoftwarePackage -ComputerName MS01-2022 -Path $SCOMIISManagementPack.FullName -CommandLine "-quiet"
#Install-LabSoftwarePackage -ComputerName MS01-2022 -Path $SCOMNETAPMManagementPack.FullName -CommandLine "-quiet"
#Get-Job -Name 'Installation of*' | Wait-Job | Out-Null

<#
Invoke-LabCommand -ActivityName 'Installing Management Packs' -ComputerName MS01-2022 -ScriptBlock {
...
Invoke-LabCommand -ActivityName 'Installing SCOM Agent(s)' -ComputerName ($SCOMServers | Where{ $_.Name -like '*01-*' }) -ScriptBlock {
	...
	{
		Install-SCOMAgent -PrimaryManagementServer $(Get-SCOMManagementServer -Name $env:COMPUTERNAME) -DNSHostName $machine
	}
	$DomainControllers = "DC-2022.$FQDNDomainName"
	<#
	function Time-Stamp
	{
...
Invoke-LabCommand -ActivityName 'Installing SCOM Agent to Domain Controller(s)' -ComputerName ($ToBecomeAgents | Where { $_.Name -like "DC*" }) -ScriptBlock {
		...
		Sleep 1
		$args = "USE_SETTINGS_FROM_AD=0 USE_MANUALLY_SPECIFIED_SETTINGS=1 MANAGEMENT_GROUP=$LabName MANAGEMENT_SERVER_DNS=MS01-2022.$FQDNDomainName MANAGEMENT_SERVER_AD_NAME=MS01-2022.$FQDNDomainName SECURE_PORT=5723 ACTIONS_USE_COMPUTER_ACCOUNT=1 AcceptEndUserLicenseAgreement=1"
...
if ($UseDefaultSwitch)
{
	...
	Get-VM -Name 'MSSQL-2022' | Remove-VMNetworkAdapter -Name 'Default Switch' -ErrorAction SilentlyContinue
}
