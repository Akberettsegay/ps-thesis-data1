...
<#
        .SYNOPSIS
		Collector to get password reset policy from Microsoft Entra ID
	...
	begin {
		...
		$monkey_metadata = @{
			...
			ResourceType = $null;
			resourceName = $null;
			collectorName = "Get-MonkeyADPasswordResetPolicy";
			ApiType = "EntraIDPortal";
			description = "Collector to get password reset policy from Microsoft Entra ID";
			Group = @(
				"EntraIDPortal"
			);
			Tags = @(
	...
	process {
		...
		if ($ad_password_reset_policy) {
			$ad_password_reset_policy.PSObject.TypeNames.Insert(0,'Monkey365.EntraID.SSPRPolicy')
			[pscustomobject]$obj = @{
			...
			$msg = @{
				MessageData = ($message.MonkeyEmptyResponseMessage -f "Microsoft Entra ID password reset policy",$O365Object.TenantID);
				callStack = (Get-PSCallStack | Select-Object -First 1);
				logLevel = "verbose";
				InformationAction = $O365Object.InformationAction;
