...
#$ComboBox_namespace.DataSource # изменение коллекции элементов невозможно, если задано свойство DataSource
$ns_items = @("root", "root\MicrosoftActiveDirectory", "root\CIMV2", "root\CIMV2\Terminalservices", "root\Microsoft", `
...
foreach ($f in $ns_items) { $ComboBox_namespace.Items.Add($f) }
$ComboBox_namespace.Text = "ROOT"
...
$TextBox_Search = New-Object System.Windows.Forms.ComboBox
$class_items = @("System", "Service", "Process", "Product", "Fix", "Device", "Board", "Disk", "Memory", "Video", "Network", "Driver", "BIOS", "Startup", "Share", `
        "Time", "User", "Account", "Event", "Terminal")
foreach ($f in $class_items) { $TextBox_Search.Items.Add($f) }
...
$button_view_class.Add_Click({
        $button_view_class.Enabled = $false
        ...
        $button_view_class.Enabled = $true

        #$ComboBox_namespace.Items.Clear()
        #$ns_items_path = @(gwmi -ComputerName $comp -Namespace $ns -class "__NAMESPACE" | select @{Label="FullPath"; Expression={$_.__namespace+"\"+$_.name}})
        #$ns_items_path | select FullPath > $path
        #$out_txt_2 = Get-Content $path
        #$out_txt_2 = $out_txt_2[3..100]
        #Remove-Item -Recurse $path
        #foreach ($f in $out_txt_2) {$ComboBox_namespace.Items.Add($f)}
    })
#endregion
...
$button_view_namespace.Add_Click({
        $button_view_namespace.Enabled = $false
        $dataGridView.AutoSizeColumnsMode = "Fill"
        main-var
        $out_namespace = @(gwmi -ComputerName $comp -Namespace $ns -class "__NAMESPACE" | select @{Label = "FullPath"; Expression = { $_.__namespace + "\" + $_.name } })
        ...
        Remove-Item -Recurse $path
        $outputBox_Tab_Text.Text = $out_txt | out-string
        $button_view_namespace.Enabled = $true
    })
#endregion
...
$button_find_object.Add_Click({
        $button_find_object.Enabled = $false
        ...
        $button_find_object.Enabled = $true
    })
#endregion
