...
.DESCRIPTION
    This script robustly starts an EC2 instance, with improved error handling and output. Compatible with AWS CLI v2.16+ (2025).
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Error "Failed to start EC2 instance: $result"
        exit $LASTEXITCODE
    }
} catch {
