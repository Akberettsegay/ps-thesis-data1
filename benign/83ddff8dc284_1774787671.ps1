...
[CmdletBinding()]
    Param (
        [Parameter(Mandatory=$true)] [string]$CoveUserName,                                                ## Backup.Management user
        [Parameter(Mandatory=$true)] [string]$CovePassword,                                                ## Backup.Management userpw
...
# (Create authentication JSON object using ConvertTo-JSON)
    ...
    Add-Member -PassThru NoteProperty params @{username=$CoveUserName;password=$CovePassword}|
