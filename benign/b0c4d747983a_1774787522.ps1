...
Write-Output "[Environments/Variables]"
...
foreach ($fe in $fresults)
{
    ...
    where
        f.name = '"+$foldername +"' and e.name='"+$envname+"'
    order by
    ...
    # Write Out
    if (@($envVresults).count -gt 0)
    {
        Write-Output ("     Variables: [{0}]" -f @($envVresults).count)
    }
