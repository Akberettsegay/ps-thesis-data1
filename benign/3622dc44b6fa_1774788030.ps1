...
Describe 'Templates' -Tag 'Template' {
    ...
    Context 'Documentation for examples' {
        ...
        It 'Generates expected output with Bicep registry schema' {
            $invokeParams = @{
                Module = 'PSDocs.Azure'
                Option = (New-PSDocumentOption -Option @{ 'CONFIGURATION.AZURE_BICEP_REGISTRY_MODULES_METADATA_SCHEMA_ENABLED' = $True })
            }

            # Generates templates
            $result = Get-AzDocTemplateFile -Path $bicepPath | ForEach-Object {
                $template = Get-Item -Path $_.TemplateFile;
                $parentPath = $template.Directory.FullName;
                $expectedContent = Get-Content -Path (Join-Path -Path $parentPath -ChildPath 'README.md') -Raw;
                $actualContent = Invoke-PSDocument @invokeParams -OutputPath $outputPath -InputPath $template.FullName -PassThru;
