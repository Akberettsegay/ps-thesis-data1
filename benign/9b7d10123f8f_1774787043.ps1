...
<#
        .SYNOPSIS
		Collector to get information about policies applied to subscription
	...
	begin {
		...
		$monkey_metadata = @{
			...
			ResourceType = $null;
			resourceName = $null;
			collectorName = "Get-MonkeyAzSecCenterPolicyForSubscription";
			ApiType = "resourceManagement";
			description = "Collector to get information about subscription policies";
			Group = @(
				"DefenderForCloud"
			);
			Tags = @(
	...
	process {
		$msg = @{
			MessageData = ($message.MonkeyGenericTaskMessage -f $collectorId,"Azure Subscription Policies",$O365Object.current_subscription.displayName);
