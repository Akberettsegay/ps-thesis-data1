...
<#
        .SYNOPSIS
		Azure Collector to get Microsoft Defender for Cloud settings
	...
	begin {
		...
		$monkey_metadata = @{
			...
			ResourceType = $null;
			resourceName = $null;
			collectorName = "Get-MonkeyAZSecCenterConfig";
			ApiType = "resourceManagement";
			description = "Collector to get settings from Microsoft Defender for Cloud";
			Group = @(
				"DefenderForCloud"
			);
			Tags = @(
	...
	process {
		$msg = @{
			MessageData = ($message.MonkeyGenericTaskMessage -f $collectorId,"Microsoft Defender for Cloud Configuration",$O365Object.current_subscription.displayName);
