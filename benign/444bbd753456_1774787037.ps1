...
<#
        .SYNOPSIS
		Collector to get classic disks info from Azure
	...
	begin {
		...
		$monkey_metadata = @{
			...
			ResourceType = $null;
			resourceName = $null;
			collectorName = "Get-MonkeyAzClassicDisk";
			ApiType = "resourceManagement";
			description = "Collector to get classic disks info from Azure";
			Group = @(
				"VirtualMachines"
			);
			Tags = @(
	...
	process {
		$msg = @{
			MessageData = ($message.MonkeyGenericTaskMessage -f $collectorId,"Azure classic disks",$O365Object.current_subscription.displayName);
