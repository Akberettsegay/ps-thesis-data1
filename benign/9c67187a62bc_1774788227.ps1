...
# Define Base Path for Unix Systems (Using XDG standard or Home)
$AppDataPath = if ($env:XDG_DATA_HOME) { $env:XDG_DATA_HOME } else { Join-Path $env:HOME ".local/share" }

if (-not (Test-Path $AppDataPath)) { New-Item -ItemType Directory -Path $AppDataPath -Force | Out-Null }

$logFolder = Join-Path $AppDataPath "PS7Logs"
...
# ──────────────────────────────
# We utilize the $AppDataPath defined at the top
$env:POSH_THEMES_PATH = Join-Path $AppDataPath "oh-my-posh-themes"
$ThemeFile = Join-Path $env:POSH_THEMES_PATH "paradox.omp.json"
...
function psx {
    ...
    $PSX_Version = "1.0.0"
    # Calculate path inside function scope for Unix
    $AppDataPath = if ($env:XDG_DATA_HOME) { $env:XDG_DATA_HOME } else { Join-Path $env:HOME ".local/share" }
    $PSX_LogFile = Join-Path $AppDataPath "PS7Logs/ps7_open_logs.json"
    $baseUrl = "https://raw.githubusercontent.com/Mahmoud-walid/psx-profile/main"
    $profileUrl = "$baseUrl/profiles/Microsoft.PowerShell_profile.macos.ps1"
