<#
	...
	.DESCRIPTION
		Export Scripts from inside of XML Management Packs. This will make it easier to troubleshoot issues with scripts in Discoveries, Monitors, or Rules.
	...
	.NOTES
		...
		Original Location: https://github.com/Udish17/SCOM-PowerShell-Scripts/blob/SCOM-PowerShell-Scripts/master/DumpScriptsfromSCOMMP.ps1
  		Author: Blake Drumm (blakedrumm@microsoft.com)
    		This script location: https://github.com/blakedrumm/SCOM-Scripts-and-SQL/blob/master/Powershell/Export-ScriptsFromMPXML.ps1
...
function Export-ScriptsFromMPXML
{
	[CmdletBinding()]
	[OutputType([string])]
	param
	...
	#region Dump-Scripts
	#This script will dump only the scripts which are defined inside Data Source Files/File
	$datasourcesScripts = (Select-Xml -Xml $xmlcontent -XPath //*/DataSource).Node | Select PsScriptName, PowerShellScriptParameters, PowerShellScriptParam, PowerShellScriptBody, cScriptName, cScriptParameters, cScriptBody | Where-Object { $_.PsScriptName -ne $null -or $_.cScriptName -ne $null }
	if ($datasourcesScripts)
	{
		foreach ($datasource in $datasourcesScripts)
		{
			if ($datasource.PsScriptName)
			{
				$scriptname = $datasource.PsScriptName
				Write-Host "Script found. Dumping Script....    " $scriptname -ForegroundColor Green
				$powershellBody = @"
`#$($datasource.PowerShellScriptParameters)
$($datasource.PowerShellScriptParam)
$($datasource.PowerShellScriptBody)
"@
				
				
				$powershellBody | Out-File $ExportLocation\$scriptname
				
			}
			if ($datasource.cScriptName)
			{
				$scriptname = $datasource.cScriptName
				Write-Host "Script found. Dumping Script....    " $scriptname -ForegroundColor Green
				$cscriptBody = @"
$($datasource.cScriptParameters)
$($datasource.cScriptBody)
"@
				
				
				$cscriptBody | Out-File $ExportLocation\$scriptname
			}
		}
	}
	#endregion Dump-Scripts
	
	#region Dump-DS-DataSource
	#This script will dump only the scripts which are defined inside Data Source Files/File
	$datasources = $xmlcontent.SelectNodes("//ManagementPack/TypeDefinitions/ModuleTypes/*[starts-with(name(), 'DataSourceModuleType')]/ModuleImplementation/Composite/MemberModules/*[starts-with(name(), 'DataSource')]")
	if ($datasources)
	{
...
if ($SourceXML -or $ExportLocation)
{
	Export-ScriptsFromMPXML -SourceXML $SourceXML -ExportLocation $ExportLocation
}
else
{
	Export-ScriptsFromMPXML
}