...
Param (
    ...
    [string]$ScanPath = "C:\Script Root\0-Script Master",
    ...
    [string]$AlwaysInclude = "C:\ProgramData\MXB\New folder|Q:\modules",
...
#region ----- Functions ----
    ...
    function Import-ClientToolModule {
        ...
        $resolvedPath = Resolve-Path $ModulePath -ErrorAction SilentlyContinue
        if (-not $resolvedPath) {
            Write-Error "ClientTool module not found at: $ModulePath"
            Write-Error "Install from: https://github.com/BackupNerd/ClientTool"
            exit 1
        }
        Import-Module $resolvedPath.Path -Force -ErrorAction Stop
